export const MyCompetencies: {
    id: number;
    title: string;
    items: {
        id: number;
        name: string;
        description: string;
        icon: string;
    }[],
}[] = [
        {
            id: 1,
            title: "Base Tecnológica para soluções",
            items: [
                {
                    id: 1,
                    name: "JavaScript",
                    description: `
                    Um site de e-commerce e um carrinho de compras, onde os usuários podem visualizar 
                    a página principal, pesquisar os produtos, adicionar produtos, selecionar quantidades,
                    visualizar os itens no carrinho, remover produtos, calcular o preço total das compras e
                    finalizar o pedido de compra.
                `,
                    icon: "/assets/icons/js.svg",
                },
                {
                    id: 2,
                    name: "TypeScript",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/ts.svg",
                },
                {
                    id: 3,
                    name: "HTML5",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/html5.svg",
                },
                {
                    id: 4,
                    name: "CSS3",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/css.svg",
                },
                {
                    id: 5,
                    name: "React",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/react.svg",
                },
            ],
        },
        {
            id: 2,
            title: "Tecnologias e ferramentas disponíveis para escolha e otimização de soluções",
            items: [
                {
                    id: 1,
                    name: "Next.js",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/nextjs.svg",
                },
                {
                    id: 2,
                    name: "Bootstrap",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/bootstrap.svg",
                },
                {
                    id: 3,
                    name: "Tailwind CSS",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/tailwindcss.svg",
                },
                {
                    id: 4,
                    name: "Ant Design",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/antDesign.svg",
                },
                {
                    id: 5,
                    name: "SASS",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/sass.svg",
                },

            ],
        },
        {
            id: 3,
            title: "Boas práticas e habilidades para melhores soluções e qualidade",
            items: [
                {
                    id: 1,
                    name: "Trabalho em equipe",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/interpessoal.svg",
                },
                {
                    id: 2,
                    name: "Desenvolvimento de testes",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/tests.svg",
                },
                {
                    id: 3,
                    name: "UX/UI Design",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/UXUI.svg",
                },
                {
                    id: 4,
                    name: "Princípios de código limpo e de arquitetura limpa",
                    description: `
                    ...
                `,
                    icon: "/assets/icons/book.svg",
                },
            ],
        },
    ];
